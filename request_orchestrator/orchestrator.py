...
def _build_run_agent_node(agent_name: str, runner: Callable) -> Callable[[MainState], MainState]:
    def _run_agent_update(state: MainState) -> MainState:
        if not _should_run_agent(state, agent_name):
            return state
        current_agent_state = state.agent_states[agent_name]
        updated_agent_state = runner(current_agent_state)
        state.agent_states[updated_agent_state.agent_profile.name] = updated_agent_state
        return state
...
def _compile_graph():
    ...
    builder.add_node(
        ...
        lambda state: (state.initialize_agent_states(), state)[1],
    )
    ...
    builder.add_edge(previous_edge, SYNTHESIZE_EDGE)
